"""Rembg generated word images into assets/words and rebuild the map.
...
DST = ROOT / "assets" / "words"


def orders_for_lesson(n: int) -> list[int]:
    import json

    data = json.loads((ROOT / "data" / "lessons" / f"bai-{n:02d}.json").read_text(encoding="utf-8"))
    return [int(w["order"]) for w in data]
...
def main() -> None:
    ...
    subprocess.check_call(["node", str(ROOT / "scripts" / "write-word-image-map.js")], cwd=ROOT)
